        ...
        <CardContent>
          <div className="space-y-4">
            {patients.map((patient) => (
              <Card key={patient.id} className="border-l-4 border-l-medical-blue">
                ...
                <CardContent>
                    ...
                    <AccordionItem value="categories">
                      ...
                      <AccordionContent>
                        <div className="space-y-4">
                          {categories.map((category) => {
                            ...
                            return (
                              <div key={category.id} className="space-y-2">
                                <h4 className="font-medium text-medical-blue">{category.name}</h4>
                                    ...
                                    <div key={test.id} className="flex items-center justify-between p-2 border rounded">
                                      ...
                                      <Button
                                        ...
                                        onClick={() => {
                                          ...
                                          handleAddTestToPatient(test);
                                        }}
                                        disabled={selectedTests.some(t => t.id === test.id) && selectedPatientForTests === patient.id}
                                      >
                                        Add
                                      </Button>
                                    </div>
                          ...
                          {selectedPatientForTests === patient.id && selectedTests.length > 0 && (
                            <div className="mt-4 p-4 bg-muted/50 rounded">
                              <h5 className="font-medium mb-2">Selected Tests:</h5>
                                ...
                                {selectedTests.map((test) => (
                                  <div key={test.id} className="flex items-center justify-between text-sm">
                                    ...
                                    <Button
                                      size="sm"
                                      variant="ghost"
                                      onClick={() => handleRemoveTestFromPatient(test.id)}
                                    >
                              ...
                              <Button
                                size="sm"
                                className="mt-2 bg-medical-green hover:bg-medical-green/90 text-white"
                                onClick={handleSaveTestsForPatient}
                              >
                  ...
                  {/* Show patient's test history */}
                  <div className="mt-4">
                    <h5 className="font-medium text-sm mb-2">Recent Tests:</h5>
                    {getPatientTests(patient.id).map((patientTest) => (
                      <Badge key={patientTest.id} variant="secondary" className="mr-2 mb-2">
                        {Array.isArray(patientTest.tests) ? patientTest.tests.length : 0} tests - {new Date(patientTest.created_at).toLocaleDateString()}
                      </Badge>
                    ))}
                  </div>
                </CardContent>
